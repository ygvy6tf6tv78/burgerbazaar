...
const headerImages = [
  { src: '/burger-bazaar-header.jpg', position: '50% 32%' },
  { src: '/burger-bazaar-header-food.jpg', position: '50% 48%' },
  { src: '/burger-bazaar-chaos-banner.jpeg', position: '50% 50%' },
]
          ...
          >
            ...
            <div className="relative h-40 bg-slate-900 overflow-hidden">
              <AnimatePresence mode="wait" initial={false}>
                ...
                >
                  <Image
                    src={headerImages[headerImageIndex].src}
                    alt="Burger Bazaar"
                    fill
                    className="object-cover"
                    style={{ objectPosition: headerImages[headerImageIndex].position }}
                    priority
              ...
              <div className="absolute bottom-2 left-1/2 z-10 flex -translate-x-1/2 gap-1.5" aria-hidden>
                {headerImages.map((image, index) => (
                  <span key={image.src} className={`h-1.5 rounded-full transition-all ${index === headerImageIndex ? 'w-5 bg-white' : 'w-1.5 bg-white/55'}`} />
                ))}
              </div>
            </div>
            ...
            >
              ...
              <motion.a
                ...
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="flex h-11 w-11 touch-manipulation items-center justify-center overflow-hidden rounded-full border-2 border-white bg-white p-2 shadow-[0_8px_16px_rgba(0,0,0,0.28)]"
                aria-label="Open Burger Bazaar on Instagram"
              >
                <Image src="/instagram-official.png" alt="Instagram" width={28} height={28} className="h-7 w-7 object-contain" />
              </motion.a>
            </motion.div>
